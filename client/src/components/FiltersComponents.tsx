...
export const FiltersComponents = ({array,filterObject, callbackFunction, widthPercent}: Props) => {
    ...
    return (
        <div style = {{ display:"flex", width: `${widthPercent}%`}}>
            ...
            <FormControl style = {{justifyContent:'flex-start', alignItems:'flex-start', width: `${100 / array.length}%`}}> 
                <InputLabel id={`demo-simple-select-label${item.filterBy}`}>{`${item.filterBy} Filter`}</InputLabel>
                <Select labelId={`demo-simple-select-label${item.filterBy}`} style = {{height:'100%',width:'60%' }} onChange={(e)=>determineWhatToSet(item.filterBy, e.target.value)}>
