...
export async function firstPairing(studentsArr: any[], mentorsArr: any[]) {
    studentsArr = await Promise.all(
        studentsArr.map(async (student: any) => { if (!student.geo) return await GeoCoding(student); else return student } )
            )
            .then((arr) => {
              return arr;
            });
    mentorsArr = await Promise.all(
        mentorsArr.map(async (mentor: any) => { if (!mentor.geo) return await GeoCoding(mentor); else return mentor })
            ).then((arr) => {
              return arr;
            });
...
export async function fixedPairing (studentsArr: any[], mentorsArr: any[]) {
    const initial = await firstPairing(studentsArr, mentorsArr);
    const final: any[] = []
    console.log(initial)
    initial.forEach((obj, index) => {
        obj.distance = getDistance(obj.student.geo.lat, obj.student.geo.lng, obj.mentor.geo.lat, obj.mentor.geo.lng)
        final.push(obj)
        console.log(`${index} started`)
        for (let i = index - 1; i >= 0; i--) {
            const currentOriginalDistance = obj.distance;
            const rivalOriginalDistance = final[i].distance;
            if (Math.abs(final[index].mentor.geo.lat - final[i].student.geo.lat) < Math.abs(final[index].student.geo.lng - final[i].student.geo.lng)) {
                console.log(` ${i} pass filter 1'`)
                const currentDemoDistance = getDistance(final[index].student.geo.lat, final[index].student.geo.lng, final[i].mentor.geo.lat, final[i].mentor.geo.lng);
                const rivalDemolDistance = getDistance(final[i].student.geo.lat, final[i].student.geo.lng, final[index].mentor.geo.lat, final[index].mentor.geo.lng);
                ...
                if (demoSum < originalSum) {
                    ...
                    console.log(currentOriginalDistance, ':', currentDemoDistance, '-------', rivalOriginalDistance, ':', rivalDemolDistance)
                    console.log(final)
                };
            }
        }
    });
    const sumI = initial.reduce((x, y) => x + y.distance, 0)
...
export  const s = [
      { id: 1, address: "tel aviv", geo: {lat: 32.0852999, lng: 34.78176759999999} },
...
export   const m = [
        { id: 1, address: "tel aviv", geo: {lat: 32.0852999, lng: 34.78176759999999} },
        { id: 2, address: "ramat gan", geo: {lat: 32.068424, lng: 34.824785} },
        { id: 3, address: "tel aviv", geo: {lat: 32.0852999, lng: 34.78176759999999} },
